...
#### Convert all values in dictionary into string
#### Convert all characters to uppercase characters
#### Remove the following pieces of text: <P>, </P>, <B>, </B>, <I>, </I>
#### Remove all the unnecessary punctuation symbols denoted in the string below (convert into spaces)
...
for key in jsondata.keys():
    ...
    for tag in remove_text:
        jsondata[key] = jsondata[key].replace(tag, '')  

    for symbol in unnecessary_punctuation:
        jsondata[key] = jsondata[key].replace(symbol, ' ')
...
for word in word_list:
    if len(word) >= 3 and word not in wordcount_dictionary.keys():
...
for key, value in wordcount_dictionary.items():
    tuple = (key, value)
    final_wordlist_of_tuples.append(tuple)
...
# 19, 17, and 21 are just the highest possible values of F, R, or C that I got from looking at the constraints
for i in range(0,19):
    for j in range(0,17):
        for k in range(0, 21):
            ...
            gold = 1000*fighters + 600*rangers + 750*clerics

            lumber = 500*fighters + 750*rangers + 300*clerics

            food = 50*fighters + 45*rangers + 75*clerics

            if gold <= 18000 and lumber <= 12000 and food <= 1500:
                z = 10*fighters + 12*rangers + 16*clerics
                list = [i, j, k, z]
                temp_winners_list.append(list)


z_list = []
for list in temp_winners_list:
    z_list.append(list[3])

final_winners = temp_winners_list[z_list.index(max(z_list))]
    
optimal_F = final_winners[0]
optimal_R = final_winners[1]
optimal_C = final_winners[2]
optimal_Z = final_winners[3]
...
def Collatz_winner(number_list):
    ...
    for num in number_list:
        if Collatz(num)['collatz_length'] >= winner[0] and Collatz(num)['max_collatz'] >= winner[1]:
            winner[0] = Collatz(num)['collatz_length']
            winner[1] = Collatz(num)['max_collatz']
            winner[2] = num
